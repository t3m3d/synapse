...
func i2pCheck() {
    print("I2P+ HTTP/HTTPS proxy: " + i2pProxyHost() + ":" + i2pProxyPort())
    if i2pProxyReady() == "1" {
        print("I2P+ endpoint check: TCP listener available (not authenticated)")
        emit "1"
    }
    print("I2P+ readiness: unavailable; start I2P+ before launching this mode")
    emit "0"
}

func i2pIdentityFolder(requested) {
    if requested == "personal" { emit "personal" }
    if requested == "work" { emit "work" }
    if requested == "school" { emit "school" }
    if requested == "creator" { emit "creator" }
    if requested == "second-account" { emit "second-account" }
    emit ""
}

func toolI2pProfileRoot() {
...
func openI2pProfile(requestedIdentity) {
    let identity = i2pIdentityFolder(requestedIdentity)
    if len(identity) == 0 {
        print("Unknown I2P identity: " + requestedIdentity)
        emit "0"
    }
    if i2pProxyReady() != "1" {
    ...
    if !exists(browser) {
        print("Synapse browser build is missing: " + browser)
        emit "0"
    }
    let root = toolI2pProfileRoot()
    if len(root) == 0 {
        print("Windows local app-data path is unavailable")
        emit "0"
    }
    let profile = joinPath(root, i2pProfileFolder(identity))
    ...
    if !contains(exec(profileCommand), "yes") {
        print("I2P profile could not be created: " + profile)
        emit "0"
    }
    if toolEnforceI2pPrefs(profile) != "1" {
        print("I2P startup policy could not be written and verified")
        emit "0"
    }
    if startSynapseBrowser(browser, profile, "i2p") != "1" {
        print("Windows could not start Synapse")
        emit "0"
    }
    print("I2P " + identity + " started through " + i2pProxyHost() + ":" + i2pProxyPort())
...
func usage() {
    ...
    print("  synapse-tool i2p-check")
    print("  synapse-tool i2p-open [personal|work|school|creator|second-account]")
...
just run {
    ...
    if command == "i2p-check" {
        if i2pCheck() == "1" { exit(0) }
        exit(1)
    }
    if command == "i2p-open" {
        let identity = "personal"
        if argCount() > 1 { identity = arg(1) }
        if openI2pProfile(identity) == "1" { exit(0) }
        exit(1)
    }
